...
const {
  ...
  SEGMENT_MAX_AGE, SEGMENT_PREFIX, SEGMENT_CLEANUP_HEARTBEAT,
  MIME_LARGE, MIME_FILE,
} = require('./env.json');
...
class SeqWriteFile {
  ...
  _register_manifest(block_hash) {
    ...
    for(let {part_hash, part_size, part_path} of this.parts) {
      ...
      await Storage.rename(this.part_path, part_path);
    }

    var etag = md5(manifest.map(line => line.etag).join(''));
    var remote_url = `${block_path}?multipart-manifest=put`;

    await Storage.putStream(this.storage_ctx, bl(JSON.stringify(manifest)), this.storage_container, remote_url, {etag});
      //maybe this can be done once ?
    let headers = {'content-type' :  MIME_LARGE};
    await Storage.update(this.storage_ctx, this.storage_container, block_path, {headers});
  }
...
console.log("WRITEINT", buf, 0, fixed, this.part_offset);
process.exit();
    ////
    this.part_fd.write(buf, (err, size) => {
