...
const SWIFT_SLO_HEADER = 'x-static-large-object';
...
class CCD {
  ...
  async _consolidate(segment_name) {
    ...
    if(!bigfile_head || !bigfile_head.headers[SWIFT_SLO_HEADER]) {
    ...
    if(!cas_head || cas_head.headers[SWIFT_SLO_HEADER]) {
      console.log("Consolidating %s into %s", segment_name, segment_path, prettyFileSize(head.headers['content-length']));
      try {
        ...
        console.log("WAITING A BIT FOR %s to consolidate", segment_path);

        await retryUntil(async () => {
          let head = await Storage.check(this.storage_ctx, this.storage_container, segment_path);
          console.log("Retryuntil", head, head && head.headers);
          return head;
        }, 60 * 10000, 10 * 1000, `Cannot consolidate ${segment_path}`);
      }
  ...
  async _updateManifest(bigfile_hash, segment_hash) {
    console.log("Updating %s manifest", bigfile_hash);
