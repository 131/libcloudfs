...
const {O_RDONLY, O_WRONLY, O_SYNC} = {O_SYNC : 0x101000, ...fs.constants}; //O_RDWR
...
class casfs {
  ...
  async mount(mountPath) {
    ...
    fuse.mount(mountPath, {
      force   : true,
      options : ['allow_other', 'sync', 'direct_io'], //for smbd
      ...
      truncate : function(path, size, cb) {
        console.log("truncate %s to %d", path, size);
        cb();
      },

      ftruncate : function(path, fd, size, cb) {
        console.log("ftruncate %s (in %d) to %d", path, fd, size);
        cb();
      },
  ...
  close() {
    console.log("SHOULD SUTDOWN", this.mountPath);
    if(!this.mountPath)
      return;
