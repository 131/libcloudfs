...
class SeqWriteFile {
  ...
  async close() {
    if(!this.part_fd)
      return {block_hash : EMPTY_HASH, block_size : 0};
  ...
  async _register_manifest(block_hash) {
    ...
    for(let {part_hash, part_size, part_path} of this.parts) {
      //wait 10s for remote part to be available
      await retryUntil(async () => {
        return Storage.check(this.storage_ctx, this.storage_container, part_path);
      }, 10 * 1000, 1000);
    ...
    await Storage.putStream(this.storage_ctx, Buffer.from(JSON.stringify(manifest)), this.storage_container, remote_url, {etag, 'content-type' :  MIME_LARGE});
  }
  ...
  write(buf, len, offset, cb) {
    ...
    logger.debug("Writing", {len, offset, fixed, remain, part_offset : this.part_offset});
